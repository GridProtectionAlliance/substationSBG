...
SET defaulttarget=%LOCALAPPDATA%\Temp\substationSBG
IF "%remote%" == "" SET remote=git@github.com:GridProtectionAlliance/substationSBG.git
...
SET targettools=%target%\Source\Applications\substationSBG\substationSBGSetup
...
FOR /R "%sourceschema%" %%x IN (GSFSchema.*) DO REN "%%x" "substationSBG.*"
FOR /R "%sourceschema%" %%x IN (GSFSchema-InitialDataSet.*) DO REN "%%x" "substationSBG-InitialDataSet.*"
FOR /R "%sourceschema%" %%x IN (GSFSchema-SampleDataSet.*) DO REN "%%x" "substationSBG-SampleDataSet.*"
...
"%replace%" /r /v "%targetschema%\*.sql" GSFSchema substationSBG
...
"%replace%" /r /v "%targetschema%\*db-update.bat" GSFSchema substationSBG
