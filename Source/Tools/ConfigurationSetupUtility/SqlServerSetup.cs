...
namespace ConfigurationSetupUtility
{
    ...
    public class SqlServerSetup
    {
        ...
        public void GrantDatabaseAccess(string loginName)
        {
            ...
            ExecuteStatement("IF DATABASE_PRINCIPAL_ID('substationSBGAdminRole') IS NULL CREATE ROLE [substationSBGAdminRole] AUTHORIZATION [dbo]");
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('{0}') IS NOT NULL AND DATABASE_PRINCIPAL_ID('substationSBGAdminRole') IS NOT NULL EXEC sp_addrolemember N'substationSBGAdminRole', N'{0}'", loginName));
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('substationSBGAdminRole') IS NOT NULL EXEC sp_addrolemember N'db_datareader', N'substationSBGAdminRole'"));
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('substationSBGAdminRole') IS NOT NULL EXEC sp_addrolemember N'db_datawriter', N'substationSBGAdminRole'"));
        }
                    ...
                    while ((object)line != null)
                    {
                        ...
                        else
                        {
                            if (trimLine.StartsWith("CREATE DATABASE") || trimLine.StartsWith("ALTER DATABASE") || trimLine.StartsWith("USE"))
                                line = line.Replace("substationSBG", DatabaseName);
